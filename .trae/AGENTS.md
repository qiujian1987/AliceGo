...
| 阶段 | 流程 | 说明 |
|------|------|------|
| 需求分析 | @feature-analyst + requirement-analyzer | 分析新增需求，创建特性文档 |
| 设计 | @architect + @frontend-designer + @dba | 更新架构和API |
| 任务拆解 | @team-lead + task-decomposition | 拆解新特性任务 |
| 测试用例 | @qa + test-case-design | 为新功能编写测试用例 |
| TDD开发 | @backend-dev + @frontend-dev | 实现新功能 |
| 代码评审 | @code-reviewer | 评审新代码 |
| 测试执行 | @qa | 执行测试验证 |
