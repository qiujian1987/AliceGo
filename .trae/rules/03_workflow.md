...
本规则定义SOLO Coder在项目开发过程中的26步标准流程。SOLO Coder作为主控，协调各专业Agent完成项目开发。

流程采用**Loop拓扑**设计，包含四个核心回环：需求评审回环、设计评审回环、测试评审回环、代码评审回环。每个回环定义了失败回退路径和最大重试次数（Stopping Rule），确保流程能够自主循环优化直至通过。
...
- **Loop拓扑（回环机制）**：
  - **成功跳转路径**：评审通过 → 进入步骤5（需求确认）
  - **失败回退路径**：评审不通过 → 回退到步骤3（特性需求分析）进行优化
  - **Stopping Rule（最大重试次数）**：最多3次迭代，超过后升级给用户决策
- **SOLO Coder操作**：
  1. **调用 @req-reviewer Agent**（必须）
  2. 等待@req-reviewer Agent完成评审
  3. 检查评审结果
  4. 如果不通过且迭代<3次，反馈给前置Agent优化，回退到步骤3重新执行
...
- **强制规则**：
  ...
  3. 两部分评审必须全部完成
- **Loop拓扑（回环机制）**：
  - **成功跳转路径**：两部分评审都通过 → 进入步骤14（设计确认）
  - **失败回退路径**：
    - 12.1 架构评审不通过 → 回退到步骤8/9/10/11（根据反馈确定具体步骤）进行修改
    - 12.2 UI/UX评审不通过 → 回退到步骤9（前端架构设计）进行修改
  - **Stopping Rule（最大重试次数）**：每部分最多3次迭代，超过后升级给用户决策
- **SOLO Coder操作**：
  ...
  6. 检查UI/UX评审结果
  7. 如果任一部分不通过且迭代<3次，反馈给相关Agent优化，回退到对应步骤重新执行
...
- **Loop拓扑（回环机制）**：
  - **成功跳转路径**：评审通过 → 进入步骤18（测试确认）
  - **失败回退路径**：评审不通过 → 回退到步骤16（测试用例设计）进行优化
  - **Stopping Rule（最大重试次数）**：最多3次迭代，超过后升级给用户决策
- **SOLO Coder操作**：
  1. **调用 @test-reviewer Agent**（必须）
  2. 等待@test-reviewer Agent完成测试评审
  3. 检查评审结果
  4. 如果不通过且迭代<3次，反馈给@qa Agent优化，回退到步骤16重新执行
  5. 如果不通过且迭代=3次，升级给用户决策
...
- **Loop拓扑（回环机制）**：
  - **成功跳转路径**：评审通过 → 继续下一个任务的代码评审，所有任务通过后进入步骤23（测试执行）
  - **失败回退路径**：评审不通过 → 回退到步骤22（代码修改与验证）进行修复
  - **Stopping Rule（最大重试次数）**：最多3次迭代，超过后升级给用户决策
- **SOLO Coder操作**：
  1. **调用 @code-reviewer Agent**（必须）
  2. 等待@code-reviewer Agent完成代码评审
  3. 检查评审结果
  4. 如果不通过且迭代<3次，反馈给开发Agent修改，回退到步骤22重新执行
  5. 如果不通过且迭代=3次，升级给用户决策
